...
frame_list = [
    ...
    for file in files 
    if file.lower().endswith(img_exts) and not file.startswith('.')
]
...
for n, frame_name in enumerate(frame_list):
    ...
    for version in range(1,n_version+1):
        ...
        feat_save_dir = osp.join(save_dir, f"sam_features_{vit_mode}", os.path.splitext(frame_name)[0])
        ...
        for mask, mask_name in zip(masks, masks_name):
            ...
            mask_processed = F.interpolate(mask_processed, size=torch.Size([64, 64]), mode="nearest")
